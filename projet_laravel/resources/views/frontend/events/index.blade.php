                                    ...
                                    <tr class="event-row">
                                        ...
                                        <td class="event-actions">
                                            <div class="action-buttons">
                                                ...
                                                <!-- Generate Flyer - only once -->
                                                @php
                                                $hasFlyer = !empty($event->flyer_generated_at) || !empty($event->flyer_path) || !empty($event->flyer_image_path);
                                                @endphp
                                                @if(!$hasFlyer)
                                                <button type="button" class="btn btn-sm btn-action btn-submit js-gen-flyer" data-id="{{ $event->id }}" title="Générer le flyer">
                                                    <span class="icon"><i class="fas fa-wand-magic-sparkles"></i></span>
                                                    <span class="spinner-border spinner-border-sm d-none" role="status"></span>
                                                </button>
                                                @else
                                                <!-- Download buttons when available -->
                                                @if(!empty($event->flyer_image_path))
                                                <a href="{{ route('events.flyer.image', $event) }}" class="btn btn-sm btn-action btn-view" title="Télécharger l'image">
                                                    <i class="fas fa-image"></i>
                                                </a>
                                                @endif
                                                @if(!empty($event->flyer_path))
                                                <a href="{{ route('events.flyer.pdf', $event) }}" class="btn btn-sm btn-action btn-view" title="Télécharger le PDF">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        document.querySelectorAll('.js-gen-flyer').forEach(btn => {
            btn.addEventListener('click', async function() {
                ...
                try {
                    ...
                    // Replace generate button with download buttons
                    const wrap = this.parentElement;
                    this.remove();
                    if (data.flyer_image_path) {
                        const aImg = document.createElement('a');
                        aImg.href = `/my-events/${id}/flyer/image`;
                        aImg.className = 'btn btn-sm btn-action btn-view';
                        aImg.title = `Télécharger l'image`;
                        aImg.innerHTML = '<i class="fas fa-image"></i>';
                        wrap.appendChild(aImg);
                    }
                    if (data.flyer_path) {
                        const aPdf = document.createElement('a');
                        aPdf.href = `/my-events/${id}/flyer/pdf`;
                        aPdf.className = 'btn btn-sm btn-action btn-view';
                        aPdf.title = `Télécharger le PDF`;
                        aPdf.innerHTML = '<i class="fas fa-file-pdf"></i>';
                        wrap.appendChild(aPdf);
                    }
                } catch (e) {
                    this.disabled = false;
